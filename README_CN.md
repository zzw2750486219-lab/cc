...
浏览器打开 `http://localhost:8000` — 前端控制台，支持实时事件流、任务历史、工作区文件查看和任务取消。
...
| 隔离性 | 按任务子目录 | `docker --cpus --memory --network` |
| 启动耗时 | 即时 | ~1s |
| 工作目录 | `$WORKSPACE_DIR/{task_id}` | 容器内 `/workspace` |
...
SSE 事件（inline）：`task.created → task.started → task.tool_call → task.tool_result → task.completed`

SSE 事件（docker）：额外包含 `sandbox.created`（沙箱容器创建）和 `sandbox.destroyed`（容器销毁）
...
- **`os.path.realpath` 路径检查** — 解析软链接后再验证，阻断 symlink 穿越攻击
- **Hook 回调异常隔离** — hook 崩了只记日志，不会拖垮整个 agent loop
- **事件驱动的任务列表** — SSE 事件实时刷新侧栏（30s 兜底），浏览器标题显示活跃任务数
